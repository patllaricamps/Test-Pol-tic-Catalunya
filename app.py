...
if st.button("VEURE RESULTAT"):
    ...
    # Econ / Auth (colors canviat: esquerra autoritària = vermell, dreta autoritària = blau)
    ax[0].set_xlim(-11, 11)
    ax[0].set_ylim(-11, 11)
    ax[0].axhline(0, color='black')
    ax[0].axvline(0, color='black')
    ax[0].add_patch(patches.Rectangle((-11, 0), 11, 11, color='red', alpha=0.1))     # Esquerra autoritària
    ax[0].add_patch(patches.Rectangle((0, 0), 11, 11, color='blue', alpha=0.1))      # Dreta autoritària
    ax[0].add_patch(patches.Rectangle((-11, -11), 11, 11, color='green', alpha=0.1)) # Esquerra llibertària
    ax[0].add_patch(patches.Rectangle((0, -11), 11, 11, color='yellow', alpha=0.1))  # Dreta llibertària
    ax[0].scatter(r['Econ'], r['Auth'], s=200, c='black')
    ax[0].set_title("Econòmic / Autoritat")
    ax[0].set_xlabel("← Esq | Dre →")
    ax[0].set_ylabel("← Lib | Auth →")

    # Nac / Cult (es manté igual)
    ax[1].set_xlim(-11, 11)
    ax[1].set_ylim(-11, 11)
