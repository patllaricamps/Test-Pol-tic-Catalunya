...
import numpy as np
import plotly.graph_objects as go   # <-- nou import
...
if st.session_state["resultats"]:
    ...
    if st.session_state["mostrar_totes"]:
        ...
        fig_plotly1.update_layout(
            ...
            xaxis=dict(title="Econòmic", range=[-11, 11], zeroline=True, zerolinecolor='black'),
            yaxis=dict(title="Autoritat", range=[-11, 11], zeroline=True, zerolinecolor='black'),
            showlegend=True
        )
        # Afegir fons de colors amb formes (shapes) o rectangles
        ...
        fig_plotly2.update_layout(
            ...
            xaxis=dict(title="Nacional", range=[-11, 11], zeroline=True, zerolinecolor='black'),
            yaxis=dict(title="Cultural", range=[-11, 11], zeroline=True, zerolinecolor='black'),
            showlegend=True
        )
